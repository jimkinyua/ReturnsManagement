...
namespace Returns.Helpers
{
    public class ReturnFormAttachmentService : IReturnFormAttachmentService
    {
        ...
        public async Task<ReturnFormAttachmentPreviewDto> PreviewAttachmentAsync(AttachReturnFormsDto request)
        {
            ...
            if (request.ApplyToAllPeriodsInYear && !string.IsNullOrEmpty(request.YearId))
            {
                periodQuery = periodQuery.Where(p => p.YearId == request.YearId);
            }
            else
            {
            ...
            foreach (var period in periods)
            {
                foreach (var form in forms)
                {
                    ...
                    var filingDeadline = CalculateFilingDeadline(
                        period.EndDate, 
                        request.FilingDeadlineDays ?? DEFAULT_FILING_DEADLINE_DAYS);
        ...
        public async Task<ReturnFormAttachmentResultDto> ConfirmAttachmentAsync(ConfirmReturnFormAttachmentDto request)
        {
            ...
            try
            {
                cacheData = JsonSerializer.Deserialize<dynamic>(cachedJson)!;
            }
            catch
            {
            ...
            try
            {
                var periodIds = JsonSerializer.Deserialize<List<string>>(cacheData.GetProperty("Periods").GetRawText());
                var formIds = JsonSerializer.Deserialize<List<string>>(cacheData.GetProperty("Forms").GetRawText());
                var originalRequest = JsonSerializer.Deserialize<AttachReturnFormsDto>(cacheData.GetProperty("Request").GetRawText());
                        ...
                        if (existing != null)
                        {
                            if (request.ForceOverwrite)
                            {
                                ...
                                existing.FilingDeadline = CalculateFilingDeadline(
                                    period.EndDate,
                                    originalRequest!.FilingDeadlineDays ?? DEFAULT_FILING_DEADLINE_DAYS);
