    ...
    public class PeriodGenerator : IPeriodGenerator
    {
        ...
        private FrequencyPreviewDto GenerateFrequencyPreview(ReportingYear year, FrequencyCatalog frequency, List<ReturnPeriods> existingPeriods)
        {
            ...
            foreach (var period in periods)
            {
                ...
                {
                    SequenceNo = period.SequenceNo.Value,
                    Name = period.Name,
        ...
        private List<ReturnPeriods> GeneratePeriodList(ReportingYear year, FrequencyCatalog frequency)
        {
            ...
            while (currentDate < year.EndDateDate)
            {
                ...
                switch (frequency.LabelStrategy.ToUpperInvariant())
                {
                    ...
                    case "QUARTER":
                        var monthsFromStart = 12 * (currentDate.Year - year.StartDate.Year)
                       + (currentDate.Month - year.StartDate.Month);
                        var quarter = monthsFromStart / 3 + 1;   // Q1–Q4 for fiscal year
                        var qStart = year.StartDate.AddMonths((quarter - 1) * 3);
                        period.EndDate = qStart.AddMonths(3).AddDays(-1);
                        period.Name = $"Q{quarter} {year.Year}";
                        break;

                    case "SEMI_ANNUAL":
                        var months = 12 * (currentDate.Year - year.StartDate.Year) + (currentDate.Month - year.StartDate.Month);
                        var half = months < 6 ? 1 : 2;
                        var hStart = year.StartDate.AddMonths(half == 1 ? 0 : 6);
                        period.EndDate = hStart.AddMonths(6).AddDays(-1);
                        period.Name = $"H{half} {year.Year}";
                        break;
                    ...
                    default:
                        if (frequency.IntervalDays <= 0)
                            throw new InvalidOperationException("IntervalDays not set.");
                        period.EndDate = currentDate.AddDays(frequency.IntervalDays - 1);
                        period.Name = $"Period {sequenceNo} {year.Year}";
                        break;
                }
